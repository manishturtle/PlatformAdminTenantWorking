...
export const UserManagement: React.FC<UserManagementProps> = ({ tenantSlug }) => {
  ...
  const t = (key: string, options?: any): string => {
    const translations: Record<string, string> = {
      ...
      'userManagement.inviteNewUser': 'Invite New User',
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/${tenantSlug}/tenant-admin/users/?page=${page}&page_size=${rowsPerPage}`,
        {
          headers: {
            ...authHeader,
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/${tenantSlug}/tenant-admin/users/${selectedUserId}/`,
        {
          method: 'DELETE',
          headers: {
            ...authHeader,
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid sx={{ gridColumn: 'span 12', '@media (min-width: 900px)': { gridColumn: 'span 4' } }}>
          <UserStatCard
            icon={<GroupIcon />}
            iconBgColor="#e8f5e9"
            iconColor="#2e7d32"
