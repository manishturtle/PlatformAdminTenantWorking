...
export default function TenantLoginPage({ params: paramsPromise }: { params: Promise<{ tenantSlug: string }> }) {
  ...
  const checkUser = async (emailToCheck: string = email) => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL?.replace(/\/$/, ''); // Remove trailing slash if present
      const response = await fetch(`http://localhost:8000/api/${tenantSlug}/tenant/auth/check-user/`, {
        method: 'POST',
        headers: {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const currentUrl = searchParams.get('currentUrl') || `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3001'}/${tenantSlug}`;
      
      const apiUrl = process.env.NEXT_PUBLIC_API_URL?.replace(/\/$/, ''); // Remove trailing slash if present
      const response = await fetch(`http://localhost:8000/api/${tenantSlug}/tenant/auth/login/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      >
        ...
        >
          <Box sx={{ mb: 3, display: 'flex', justifyContent: 'center' }}>
            {loginConfig?.logo ? (
              <img 
                src={`${process.env.NEXT_PUBLIC_API_URL?.replace(/\/$/, '') || 'http://localhost:8000'}${loginConfig.logo}`} 
          ...
          <Typography component="h1" variant="h5" sx={{ mb: 3 }}>
            {loginConfig?.brand_name || (userExists ? 'Sign in' : 'Welcome')}
          </Typography>
          ...
          {userExists ? (
            ...
            <Box component="form" onSubmit={handleCheckUser} noValidate sx={{ mt: 1, width: '100%' }}>
              <TextField
                margin="normal"
                required
                fullWidth
