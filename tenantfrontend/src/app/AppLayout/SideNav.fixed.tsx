...
export default function SideNav({ isOpen, onClose, userType, isAuthenticated, isLoginPage = false }: SideNavProps) {
  ...
  const menuSections: MenuSection[] = useMemo(() => {
    ...
    if (userType === 'platform-admin') {
      return [{
        ...
        items: [
          { text: 'Dashboard', icon: <DashboardIcon />, path: '/platform-admin/dashboard' },
          { text: 'Tenants', icon: <BusinessIcon />, path: '/platform-admin/tenants' },
          { text: 'Users', icon: <PeopleIcon />, path: '/platform-admin/users' },
          { text: 'Billing & Plans', icon: <PlansIcon />, path: '/platform-admin/billing-plans' },
          { text: 'Security', icon: <SecurityOutlined />, path: '/platform-admin/security' },
          { text: 'System Settings', icon: <SettingsIcon />, path: '/platform-admin/settings' },
        ]
      }];
    } else {
      ...
      return [{
        ...
        icon: <AdminPanelSettingsOutlined />,
        items: [
          { text: 'Dashboard', icon: <DashboardIcon />, path: `/${tenantSlug}/tenant-admin/dashboard` },
          { text: 'User Management', icon: <PeopleIcon />, path: `/${tenantSlug}/tenant-admin/users` },
          { text: 'Billing', icon: <ReceiptIcon />, path: `/${tenantSlug}/tenant-admin/billing` },
          { text: 'Apps & Integrations', icon: <AppsIcon />, path: `/${tenantSlug}/tenant-admin/integrations` },
          { text: 'Theme Configuration', icon: <ColorLens />, path: `/${tenantSlug}/tenant-admin/theme-configuration` },
          { text: 'Settings', icon: <SettingsIcon />, path: `/${tenantSlug}/tenant-admin/settings` },
        ]
      }];
    }
  }, [userType, pathname]);
  ...
  const handleNavigation = useCallback((path: string) => {
    if (!isAuthenticated && !isLoginPage) {
      ...
      // redirect to login or handle differently
      return;
    }
  ...
  }, [isMobile, onClose, router, isAuthenticated, isLoginPage]);
