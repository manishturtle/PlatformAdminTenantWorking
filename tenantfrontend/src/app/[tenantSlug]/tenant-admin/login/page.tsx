...
export default function TenantAdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Check user
      const checkRes = await fetch(`http://localhost:8000/api/${tenantSlug}/tenant-admin/auth/check-user/`, {
        method: 'POST',
      ...
      // 2. Login
      const loginRes = await fetch(`http://localhost:8000/api/${tenantSlug}/tenant-admin/auth/login/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        <CardContent>
          <Box display="flex" alignItems="center" mb={2}>
            ...
            <Typography variant="h4" fontWeight="bold">
              {tenantSlug ? `${tenantSlug.toUpperCase()} Admin Login` : 'Tenant Admin Login'}
            </Typography>
          </Box>
